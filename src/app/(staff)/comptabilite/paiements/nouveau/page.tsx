...
export default function NewPaymentPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/comptabilite/paiements/${payment.id}`);
    } catch (failure) {
  ...
  return (
    <div>
      <Link href="/comptabilite/paiements" className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted hover:text-foreground">
      ...
      {statement && (
        <div className="space-y-6">
          <section aria-labelledby="statement-heading">
            ...
            <TuitionStatementView statement={statement} receiptHref={(id) => `/comptabilite/paiements/${id}`} />
          </section>

          {statement.installments.length === 0 && (
            <Alert>
              Aucune échéance : fixez d&apos;abord la scolarité mensuelle de la classe dans{" "}
              <Link href="/comptabilite/frais" className="font-medium underline">
                Frais de scolarité
              </Link>
