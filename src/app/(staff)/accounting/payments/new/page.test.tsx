...
import NewPaymentPage from "@/app/(staff)/accounting/payments/new/page";
...
describe("NewPaymentPage", () => {
  ...
  it("records the payment with the chosen formula and opens its receipt", async () => {
    ...
    await waitFor(() => expect(mockRouter.push).toHaveBeenCalledWith("/accounting/payments/p1"));
  ...
  it("points the accountant to the fees page when the class has no tuition yet", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Frais de scolarité" })).toHaveAttribute("href", "/accounting/fees");
  });
